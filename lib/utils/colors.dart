// ignore_for_file: prefer_const_declarations, prefer_const_constructors

import 'package:flutter/material.dart';

class ColorsContants {
  static final Color textColor = Color.fromRGBO(246, 243, 242, 1);
  static final Color mainBackgroundColor = Color.fromARGB(255, 11, 13, 13);
  static final Color iconColor = Color.fromRGBO(246, 243, 242, 1);
  static final Color smallTextColor = Color.fromARGB(255, 55, 45, 42);
  static final Color buttonBackgroundColor = Color(0xFFf7f6f4);
  static final Color signColor = const Color(0xFFa9a29f);
  static final Color titleColor = const Color(0xFF5c524f);
  static final Color mainBlackColor = const Color(0xFF332d2d);
  static final Color yellowColor = const Color(0xFFffd379);

 

}
 List<BoxShadow> CustomboxShadow = [
    BoxShadow(
      blurRadius: 10,
      color: Colors.white10,
      spreadRadius: 4,
      offset: Offset(-1, -1),
      blurStyle: BlurStyle.outer,
    ),

        BoxShadow(
      blurRadius: 10,
      color: Colors.white10,
      spreadRadius: 4,
      offset: Offset(2, 5),
      blurStyle: BlurStyle.outer,
    ),

  ];
